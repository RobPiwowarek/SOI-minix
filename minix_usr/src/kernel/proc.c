...
int GROUP_TIMES[GROUP_NUMBER] = {1, 2, 3}; /* table with group times, index represent group number 0 - GROUP_A, 1 - GROUP_B etc. */
...
FORWARD _PROTOTYPE( int get_next_group, (int group) );
...
int i, time, next_group;

if (rdy_head[USER_Q] == NIL_PROC) return;

head = rdy_head[USER_Q];
head->counter += 1;
next_group = head->group;

switch(next_group){
case 1: time = GROUP_TIMES[1]; break; /* grp B */
case 2: time = GROUP_TIMES[2]; break; /* grp C */
default: time = GROUP_TIMES[DEFAULT_GROUP]; /* grp A */
}

if (head->counter <= time){
pick_proc();
return;
}
...
for (i = 0; i < 3; ++i){
next = head;
next_group = get_next_group(next_group);

while (next->group != next_group && next != NIL_PROC){
...
PRIVATE int get_next_group(group)
int group;
{
if (group >= GROUP_NUMBER || group < 0) return EINVAL;

group = (group + 1) % GROUP_NUMBER;
return group;
}

PUBLIC int get_time(group_index)
int group_index;
{
if (group_index >= GROUP_NUMBER || group_index < 0) return EINVAL; 

return GROUP_TIMES[group_index];
}

PUBLIC int set_time(group_index, time)
int group_index;
int time;
{
if (group_index >= GROUP_NUMBER || group_index < 0 || time < 0) return EINVAL;

GROUP_TIMES[group_index] = time;
}
