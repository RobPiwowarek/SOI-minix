...
FORWARD _PROTOTYPE( int do_gettime, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_settime, (message *m_ptr) );   
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_GETTIME: r = do_gettime(&m); break;
	    case SYS_SETTIME: r = do_settime(&m); break;
...
PRIVATE int do_settime(m_ptr)
message *m_ptr;
{
printf("TIMES: %d %d\n", m_ptr->m1_i1, m_ptr->m1_i2); 

return  set_time(m_ptr->m1_i1, m_ptr->m1_i2); 
}

PRIVATE int do_gettime(m_ptr)
message *m_ptr;
{
return get_time(m_ptr->m1_i1);
}
