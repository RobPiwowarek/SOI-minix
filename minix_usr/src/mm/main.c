...
PUBLIC int do_gettime(){
message m = mm_in;
return _taskcall(SYSTASK, SYS_GETTIME, &m);
}

PUBLIC int do_settime(){
message m = mm_in;
return _taskcall(SYSTASK, SYS_SETTIME, &m);
}

int do_getprocnr(){
  int i; 
  for (i = 0; i < NR_PROCS; i++){
      if (mproc[i].mp_flags & IN_USE && pid == mproc[i].mp_pid) {
           return i;
      }
  }
  return ENOENT;
